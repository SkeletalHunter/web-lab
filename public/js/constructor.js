...
MyForm.prototype.getDOM = function () {
  let handler = (event) => {
    ...
    this.settings.needToDo = parseInt(
      this.DOM.settingsForm.querySelector('select[name="amount-subjects"]')
        .value,
    );
    this.settings.countDays = parseInt(
      this.DOM.settingsForm.querySelector('select[name="amount-day"]').value,
    );
...
MyForm.prototype.createDOM = function () {
  this.DOM.settingsForm.querySelector('select[name="amount-subjects"]').value =
    this.settings.needToDo;
  this.DOM.settingsForm.querySelector('select[name="amount-day"]').value =
    this.settings.countDays;
  ...
  this.arrDays.forEach((dayItem, index) => {
    ...
    dayItem.forEach((actionItem, index) => {
      day
        ...
        .appendChild(this.createActionNode(index, actionItem));
    });
...
MyForm.prototype.createActionNode = function (index, content) {
  ...
  action.innerHTML = `
    ...
    `;
  action
    .querySelector('.day-item__index')
    .appendChild(document.createTextNode(++index));
  action
    .querySelector('.day-item__content')
...
MyForm.prototype.add = function (event) {
  ...
  this.DOM.arrDays
    ...
    [day].appendChild(
      this.createActionNode(this.arrDays[day].length - 1, text),
    );
  this.IsDisable();
  this.save();
...
MyForm.prototype.IsDisable = function () {
  this.arrDays.forEach((weekItem, index) => {
    if (weekItem.length === this.settings.needToDo) {
      this.DOM.arrDays
        .querySelectorAll('.day')
        [index].querySelector('input').disabled = true;
    }
  });
};
